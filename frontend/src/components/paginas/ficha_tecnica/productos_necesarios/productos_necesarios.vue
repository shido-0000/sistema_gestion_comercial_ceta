<template >
  ...
  <!--<div class="containerPrueba">-->
    <h1>Lista de los productos necesarios</h1>
    <br>
    ...
    <table class="table" style="font-family: 'Bahnschrift'">
      ...
      <tr v-for="(item, index) in lista_productos_necesarios" :key="index">
...
export default {
  data() {
    return {
      lista_productos_necesarios: [], // Array para almacenar los datos de la API
     ...
     // popupItemNuevo: null,
      ...
      /////para filtrar
      lista_productos_necesarios_original: [], // Lista original
  ...
  mounted() {
    this.obtenerProductos_necesarios();
  ...
  methods: {
    obtenerProductos_necesarios() {
      axios.get('http://127.0.0.1:8000/api/productos_necesarios/0')
          .then(response => {

            this.lista_productos_necesarios = response.data["productos_necesarios"];
            this.lista_productos_necesarios_original = response.data["productos_necesarios"];


            // this.lista_productos_necesarios_original = response.data["productos_necesarios"];
         //   this.lista_productos_necesarios = [...this.lista_productos_necesarios_original]; // Copia la lista original a la lista filtrada
          })
          .catch(error => {
            console.error(error);
          });
    ...
    eliminar(id){
      ...
      const url = `http://127.0.0.1:8000/api/productos_necesarios/delete/${id}`;
      axios.delete(url)
          .then(response => {
            this.obtenerProductos_necesarios();
    ...
    guardarCambios() {
      ...
      const url = `http://127.0.0.1:8000/api/productos_necesarios/put/${this.popupItem.id}`;
      const payload = {
      ...
      axios.put(url, payload)
          .then(response => {
            this.obtenerProductos_necesarios(); // Actualizar la tabla si es necesario
    ...
    guardar_nuevo() {
      ...
      const url = 'http://127.0.0.1:8000/api/productos_necesarios/post/';
      const payload = {
    ...
    ordenarPor(columna) {
      ...
      this.lista_productos_necesarios.sort((a, b) => {
    ...
    aplicarFiltro() {
      if (this.quitarFiltros) {
        ...
        this.lista_productos_necesarios = [...this.lista_productos_necesarios_original];
      } else {
        // Aplica el filtro por nombre
        const filteredList = this.lista_productos_necesarios_original.filter(producto => {
          const nombreMatch = (!this.filtroNombre || producto.nombre_completo.includes(this.filtroNombre));
          return nombreMatch;
        });

        this.lista_productos_necesarios = filteredList;
      }
    },
