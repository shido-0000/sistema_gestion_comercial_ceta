<template >
  ...
  <!--<div class="containerPrueba">-->
    <h1>Lista de tareas detalladas</h1>
    <br>
    ...
    <table class="table" style="font-family: 'Bahnschrift'">
      ...
      <tr v-for="(item, index) in lista_tareas_detalladas" :key="index">
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup" style="font-family: 'Bahnschrift'">
      <div class="popup-contenidoPlazos" style="top: 33%">
        ...
        <!-- Grupo 1 de campos -->
          ...
          <div >
            <h5>Nombre de la tarea:</h5>
            <input type="text" class="form-control" name="modificar_nombre" id="modificar_nombre" v-model="popupItem.nombre_completo" required @input="validarNombre(popupItem)">
          </div>
...
export default {
  data() {
    return {
      lista_tareas_detalladas: [], // Array para almacenar los datos de la API
     ...
     // popupItemNuevo: null,
      ...
      /////para filtrar
      lista_tareas_detalladas_original: [], // Lista original
  ...
  methods: {
      ...
      axios.get('http://127.0.0.1:8000/api/tareas_detalladas/0')
          .then(response => {
           ...
           // this.lista_tareas_detalladas_original = response.data["tareas_detalladas"];
            this.lista_tareas_detalladas = response.data["tareas_detalladas"];
           this.lista_tareas_detalladas_original = response.data["tareas_detalladas"]; // Copia la lista original a la lista filtrada
          })
          .catch(error => {
            console.error(error);
          });
    ...
    eliminar(id){
      ...
      const url = `http://127.0.0.1:8000/api/tareas_detalladas/delete/${id}`;
      axios.delete(url)
          .then(response => {
            this.obtenerTareas();
    ...
    guardarCambios() {
      ...
      const url = `http://127.0.0.1:8000/api/tareas_detalladas/put/${this.popupItem.id}`;
      const payload = {
      ...
      axios.put(url, payload)
          .then(response => {
            this.obtenerTareas(); // Actualizar la tabla si es necesario
    ...
    guardar_nuevo() {
      ...
      const url = 'http://127.0.0.1:8000/api/tareas_detalladas/post/';
      const payload = {
    ...
    ordenarPor(columna) {
      ...
      this.lista_tareas_detalladas.sort((a, b) => {
    ...
    aplicarFiltro() {
      if (this.quitarFiltros) {
        ...
        //this.lista_tareas_detalladas = [...this.lista_tareas_detalladas_original];
      } else {
        // Aplica el filtro por nombre

       console.log(this.lista_tareas_detalladas_original)
        const filteredList = this.lista_tareas_detalladas_original.filter(tarea => {
          const nombreMatch = (!this.filtroNombre || tarea.nombre_completo.includes(this.filtroNombre));
          return nombreMatch;
        });

        this.lista_tareas_detalladas = filteredList;
      }
    },
