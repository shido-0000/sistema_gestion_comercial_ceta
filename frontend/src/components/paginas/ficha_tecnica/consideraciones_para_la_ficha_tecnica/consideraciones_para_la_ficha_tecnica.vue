<template >
  ...
  <!--<div class="containerPrueba">-->
    ...
    <table class="table" style="font-family: 'Bahnschrift'">
      ...
      <tr v-for="(item, index) in lista_consideraciones_para_la_ficha_tecnica" :key="index">
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup_de_Insertar" style="font-family: 'Bahnschrift'">
      <div class="popup-contenidoPlazos">
        ...
        <div class="form-group col" style="align-items: baseline;">
          <div class="col-md-12">
            <h5>Descripción de la consideración:</h5>
...
export default {
  data() {
    return {
      lista_consideraciones_para_la_ficha_tecnica: [], // Array para almacenar los datos de la API
     ...
     // popupItemNuevo: null,
      ...
      /////para filtrar
      lista_consideraciones_para_la_ficha_tecnica_original: [], // Lista original
  ...
  mounted() {
    this.obtenerConsideraciones_para_la_ficha_tecnica();
  ...
  methods: {
    obtenerConsideraciones_para_la_ficha_tecnica() {
      axios.get('http://127.0.0.1:8000/api/consideraciones_para_la_ficha_tecnica/0')
          .then(response => {
            this.lista_consideraciones_para_la_ficha_tecnica = response.data["consideraciones_para_la_ficha_tecnica"];

            //this.lista_consideraciones_para_la_ficha_tecnica_original = response.data["consideraciones_para_la_ficha_tecnica"];
            //this.lista_consideraciones_para_la_ficha_tecnica = [...this.lista_consideraciones_para_la_ficha_tecnica_original]; // Copia la lista original a la lista filtrada
          })
          .catch(error => {
            console.error(error);
          });
    ...
    eliminar(id){
      ...
      const url = `http://127.0.0.1:8000/api/consideraciones_para_la_ficha_tecnica/delete/${id}`;
      axios.delete(url)
          .then(response => {
            this.obtenerConsideraciones_para_la_ficha_tecnica();
    ...
    guardarCambios() {
      ...
      const url = `http://127.0.0.1:8000/api/consideraciones_para_la_ficha_tecnica/put/${this.popupItem.id}`;
      const payload = {
      ...
      axios.put(url, payload)
          .then(response => {
            this.obtenerConsideraciones_para_la_ficha_tecnica(); // Actualizar la tabla si es necesario
    ...
    guardar_nuevo() {
      ...
      if (this.popupItemNuevo.descripcion === ''
      ...
      const url = 'http://127.0.0.1:8000/api/consideraciones_para_la_ficha_tecnica/post/';
      const payload = {
    ...
    ordenarPor(columna) {
      ...
      this.lista_consideraciones_para_la_ficha_tecnica.sort((a, b) => {
    ...
    aplicarFiltro() {
      if (this.quitarFiltros) {
        ...
        this.lista_consideraciones_para_la_ficha_tecnica = [...this.lista_consideraciones_para_la_ficha_tecnica_original];
      } else {
        // Aplica el filtro por nombre
        const filteredList = this.lista_consideraciones_para_la_ficha_tecnica_original.filter(consideraciones => {
          const nombreMatch = (!this.filtroNombre || consideraciones.descripcion.includes(this.filtroNombre));
          return nombreMatch;
        });

        this.lista_consideraciones_para_la_ficha_tecnica = filteredList;
      }
    },
