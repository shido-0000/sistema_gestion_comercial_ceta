...
<!--  <div class="containerPrueba">-->
    ...
    <div class="form-group row" style="align-items: baseline;">
      <div class="col-md-4">
        <label for="filtroArea">Filtrar por tarea:</label>
        <select class="form-control" id="filtroNombre" v-model="filtroNombre" @change="aplicarFiltro">
          <option value="">Todos</option> <!-- Opción para mostrar todos -->
          <option v-for="tarea in lista_tareas_detalladas_aux" :key="tarea.id" :value="tarea.nombre_completo">{{ tarea.nombre_completo }}</option>
        </select></div>
    ...
    <table class="table" style="font-family: 'Bahnschrift'">
      ...
      <tr>
        ...
        <th scope="col" style="font-weight: bold"   colspan="2" id="nombre">
          Número de la tarea
          <span class="arrow" @click="ordenarPor('fields.numero_de_la_tarea')">
    &#8593;&#8595;
  </span>
        </th>
        <th scope="col" style="font-weight: bold"  >
          Nombre de la tarea
          <span class="arrow" @click="ordenarPor('fields.nombre_completo_de_la_tarea')">
    &#8593;&#8595;
  </span>
        </th>
        <th scope="col" style="font-weight: bold"   colspan="2">Listado de encuentros por semana</th>
      </tr>
      </thead>
      <tbody class="table-group-divider">
      <tr v-for="(item, index) in lista_cronograma_para_ficha_tecnica" :key="index">
        <th scope="row">{{ index + 1 }}</th>
        <td  colspan="2">{{ item.fields.numero_de_la_tarea }}</td>
        <td  >{{ item.fields.nombre_completo_de_la_tarea }}</td>
        <td   >
          <ul>
            <li v-for="id_encuentro in item.fields.lista_encuetros_por_semana" >{{ arreglar_dias_por_semana(id_encuentro) }}</li>
          </ul>
        </td>
        ...
        <td>
          <div class="fila">
            ...
            <button class="btn btn-icon-text elemento2" @click="eliminar(item.pk)" style="margin-left: 20px; border-color: red; background-color: red" type="submit">
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup_de_Insertar" style="font-family: 'Bahnschrift'">
      <div class="popup-contenidoArea">
        ...
        <h5>Número de la tarea:</h5>

        <select single class="form-control" id="validationCustom01" v-model="popupItemNuevo.numero_de_la_tarea" required @click="obtenerNombre_de_Tarea(popupItemNuevo.numero_de_la_tarea,popupItemNuevo)">
          <option v-for="tarea in lista_tareas_detalladas_aux" :key="tarea.id" :value="tarea.id">{{ tarea.id }}</option>
        </select>

        <br>
        <h5>Nombre de la tarea:</h5>

        <input type="text"  readonly class="form-control" id="validationCustom01" v-model="popupItemNuevo.nombre_completo_de_la_tarea" required>
        <br>
        <div>
          <h5>Lista de  encuentros por semana:</h5>
          <select multiple class="form-control" id="validationCustom01" v-model="selectedEncuetros_por_semana" required>
            <option v-for="encuentro in lista_encuetros_por_semana_aux" :key="encuentro.id" :value="encuentro.id">{{ encuentro.cantidad }}</option>
          </select>
        </div>
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup" style="font-family: 'Bahnschrift'">
      <div class="popup-contenidoArea">
        ...
        <select single class="form-control" id="validationCustom01" v-model="popupItem.numero_de_la_tarea" required @click="obtenerNombre_de_Tarea(popupItem.numero_de_la_tarea,popupItem)">
          <option v-for="tarea in lista_tareas_detalladas_aux" :key="tarea.id" :value="tarea.id">{{ tarea.id }}</option>
        </select>
        ...
        <input type="text"  readonly class="form-control" id="validationCustom01" v-model="popupItem.nombre_completo_de_la_tarea" required>
        <br>
        <div>
          <h5>Lista de  encuentros por semana:</h5>
          <select multiple class="form-control" id="validationCustom01" v-model="selectedEncuetros_por_semana" required>
...
export default {
  ...
  data() {
    return {
      lista_cronograma_para_ficha_tecnica: [], // Array para almacenar los datos de la API
      ...
      /////para filtrar
      lista_cronograma_para_ficha_tecnica_original: [], // Lista original de usuarios
  ...
  mounted() {
    this.obtenerCronograma_para_ficha_tecnica();
    this.obtenerLista_encuetros_por_semana();
    this.obtenerTareas();
  ...
  methods: {
    obtenerCronograma_para_ficha_tecnica() {
      axios.get('http://127.0.0.1:8000/api/cronograma_para_ficha_tecnica/0')
          .then(response => {
             this.lista_cronograma_para_ficha_tecnica = JSON.parse(response.data["cronograma_para_ficha_tecnica"]);
             this.lista_cronograma_para_ficha_tecnica_original = JSON.parse(response.data["cronograma_para_ficha_tecnica"]);
    ...
    eliminar(id){
      ...
      const url = `http://127.0.0.1:8000/api/cronograma_para_ficha_tecnica/delete/${id}`;
      console.log(id)
      axios.delete(url)
          .then(response => {
            this.obtenerCronograma_para_ficha_tecnica();
    ...
    if (this.popupItem.fields.numero_de_la_tarea === 0 ||
        this.popupItem.fields.nombre_completo_de_la_tarea === '' ||
        this.selectedEncuetros_por_semana.length === 0
      ...
      const url = `http://127.0.0.1:8000/api/cronograma_para_ficha_tecnica/put/${this.popupItem.pk}`;
      const payload = {
       // id_trabajador: this.popupItem.id_trabajador,
        numero_de_la_tarea: this.popupItem.numero_de_la_tarea,
        nombre_completo_de_la_tarea: this.popupItem.nombre_completo_de_la_tarea,
        lista_encuetros_por_semana: this.selectedEncuetros_por_semana,
        // Otros campos a modificar si es necesario
      };

      axios.put(url, payload)
          .then(response => {
            this.obtenerCronograma_para_ficha_tecnica(); // Actualizar la tabla si es necesario
    ...
    guardar_nueva() {
      if (this.popupItemNuevo.numero_de_la_tarea === 0 ||
          this.popupItemNuevo.nombre_completo_de_la_tarea === '' ||
          this.selectedEncuetros_por_semana.length === 0
      ...
      const url = 'http://127.0.0.1:8000/api/cronograma_para_ficha_tecnica/post/';
      const payload = {

        numero_de_la_tarea: this.popupItemNuevo.numero_de_la_tarea,
        nombre_completo_de_la_tarea: this.popupItemNuevo.nombre_completo_de_la_tarea,
        lista_encuetros_por_semana: this.selectedEncuetros_por_semana, // Usar los IDs de los encuentros seleccionados
    ...
    obtenerLista_encuetros_por_semana(){
      axios.get('http://127.0.0.1:8000/api/encuetros_por_semana/0')
          .then(response => {
            // this.tipos_productos = response.data["Tipos de productos"]; // Actualizar los datos en la tabla
            //  console.log(response);
           // this.lista_encuetros_por_semana_aux = JSON.parse(response.data["encuetros_por_semana"]);

            this.lista_encuetros_por_semana_aux = response.data["encuetros_por_semana"];
    ...
    obtenerCantidad_encuentros(id) {

      //console.log("id_plazo: "+id_plazo)
      if (this.lista_encuetros_por_semana_aux.length > 0) {
        const encuentro = this.lista_encuetros_por_semana_aux.find(p => p.id === id);

        if (encuentro) {
          return encuentro.cantidad;
        }
      }
      return 'Encuentros por semana no encontrado';
    },
    obtenerSemana_encuentros(id) {
      ...
      if (this.lista_encuetros_por_semana_aux.length > 0) {
        ...
        if (encuentro) {
          return encuentro.semana;
        }
      }
      return 'Encuetros por semana no encontrado';
    },

    obtenerTareas() {
      axios.get('http://127.0.0.1:8000/api/tareas_detalladas/0')
          .then(response => {

            // this.lista_tareas_detalladas_original = response.data["tareas_detalladas"];
            this.lista_tareas_detalladas_aux = response.data["tareas_detalladas"];
            // this.lista_tareas_detalladas = [...this.lista_tareas_detalladas_original]; // Copia la lista original a la lista filtrada
          })
          .catch(error => {
            console.error(error);
          });
    ...
    obtenerNombre_de_Tarea(id,item){
      const tarea = this.lista_tareas_detalladas_aux.find(p => p.id === id);
      if (tarea) {
       // return tarea.nombre_completo;
        item.nombre_completo_de_la_tarea = tarea.nombre_completo;
      }
      return 'Tarea no encontrada';

    },

    ///// funcion para corregir los dias por semana

    arreglar_dias_por_semana(id_encuentro){
      let cantidad = this.obtenerCantidad_encuentros(id_encuentro);
      let semana = this.obtenerSemana_encuentros(id_encuentro);

      return "Hay "+cantidad+" encuentros en la semana "+semana;
    },
...
///////////// funcione para validar
   ...
   /* ordenarPor(columna) {
    ...
    ordenarPor(columna) {
      ...
      // Lógica de ordenamiento aquí
      this.lista_cronograma_para_ficha_tecnica.sort((a, b) => {
        const factor = this.ordenAsc ? 1 : -1;
    ...
    aplicarFiltro() {
      if (this.quitarFiltros) {
       ...
       // this.lista_area_participante = [...this.lista_area_participante_original];
        ...
        const filteredList = this.lista_cronograma_para_ficha_tecnica_original.filter(cronograma => {

          const cronograma_Match = (!this.filtroNombre || cronograma.fields.nombre_completo_de_la_tarea === this.filtroNombre);
          return cronograma_Match;
        });

        this.lista_cronograma_para_ficha_tecnica = filteredList;
      }
    },
