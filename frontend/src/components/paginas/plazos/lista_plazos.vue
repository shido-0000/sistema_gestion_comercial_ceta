    ...
    <table class="table" style="font-family: 'Bahnschrift'">
      ...
      <!--  <th scope="col" style="font-weight: bold" align="center">Nombre</th>-->
        ...
        <th scope="col" style="font-weight: bold"  >
          Nombre
          <span class="arrow" @click="ordenarPor('nombre_plazo')">
              &#8593;&#8595;
            </span>
        </th>
        <!-- <th scope="col" style="font-weight: bold" align="center">Remuneracion</th>-->
         <th scope="col" style="font-weight: bold"   id="Remuneracion">
           Remuneración
           <span class="arrow" @click="ordenarPorValor('remuneracion')">
     &#8593;&#8595;
   </span>
         </th>
         <th scope="col" style="font-weight: bold"  >Cumplimiento de normas</th>

      <!--   <th scope="col" style="font-weight: bold" align="center">Total Asignado</th>-->
         <th scope="col" style="font-weight: bold"   id="Total_Asignado">
           Total Asignado
           <span class="arrow" @click="ordenarPorValor('total_asignado')">
     &#8593;&#8595;
   </span>
         </th>
       <!--  <th scope="col" style="font-weight: bold" align="center">Porcentaje</th>-->
         <th scope="col" style="font-weight: bold"  id="Porcentaje">
           Porcentaje
           <span class="arrow" @click="ordenarPorValor('porcentaje')">
     &#8593;&#8595;
   </span>
         </th>
      ...
      <tr v-for="(item, index) in lista_plazos" :key="index">
        <th scope="row">{{ index + 1 }}</th>
        <td  >{{ item.nombre_plazo }}</td>
        <td  >{{ item.remuneracion }}</td>
        <td  >
          <span class="checkbox-display">{{ item.cumplimiento_de_normas ? 'Sí' : 'No' }}</span>
        </td>
        ...
        <td>
          <div class="fila">
            ...
            <button class="btn btn-icon-text elemento2" @click="eliminar_plazos(item.id)" style="margin-left: 20px;border-color: red; background-color: red" type="submit">
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup_de_Insertar" style="font-family: 'Bahnschrift'">
      <div class="popup-contenidoPlazos">
        ...
        <div class="form-group row" style="align-items: baseline;">
          <div class="col-md-4">
            <h5>Nombre del plazo:</h5>
            <input type="text" class="form-control" name="insertar_nombre" id="insertar_nombre" v-model="popupItemNuevo.nombre_plazo" required @input="validarNombre(popupItemNuevo)">
          </div>
          <div class="col-md-4">
            <h5>Remuneracion:</h5>
          ...
          <div class="col-md-4">
            <h5>Cumplimiento de normas:</h5>
            <input type="checkbox" class="form-check-input" name="insertar_cumplimiento"  id="insertar_cumplimiento" v-model="popupItemNuevo.cumplimiento_de_normas">
          </div>
        </div>
        ...
        <div class="form-group row" style="align-items: baseline;">
          <div class="col-md-4">
            ...
            <input type="number" class="form-control" name="insertar_total_asignado" id="insertar_total_asignado" v-model="popupItemNuevo.total_asignado" required   min="1">
          </div>
          <div class="col-md-4">
            <h5>Porcentaje:</h5>
            <input type="number" class="form-control" name="insertar_porcentaje" id="insertar_porcentaje" v-model="popupItemNuevo.porcentaje" required    max="100"  min="1">
          </div>
        </div>

        <br>
        <div class="fila">
          <button class="btn btn-icon-text elemento3" @click="guardar_nuevo_plazo" type="submit">
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup" style="font-family: 'Bahnschrift'">
      <div class="popup-contenidoPlazos" style="top: 33%">
        ...
        <div class="form-group row" style="align-items: baseline;">
          <div class="col-md-4">
            <h5>Nombre del plazo:</h5>
            <input type="text" class="form-control" name="modificar_nombre" id="modificar_nombre" v-model="popupItem.nombre_plazo" required @input="validarNombre(popupItem)">
          </div>
          <div class="col-md-4">
            <h5>Remuneracion:</h5>
          ...
          <div class="col-md-4">
            <h5>Cumplimiento de normas:</h5>
            <input type="checkbox" class="form-check-input" name="modificar_cumplimiento" id="modificar_cumplimiento" v-model="popupItem.cumplimiento_de_normas">
          </div>
        </div>
        ...
        <div class="form-group row" style="align-items: baseline;">
          <div class="col-md-4">
            <h5>Total Asignado:</h5>
            <input type="number" class="form-control" name="modificar_total_asignado" id="modificar_total_asignado" v-model="popupItem.total_asignado" required min="1">
          </div>
          <div class="col-md-4">
            <h5>Porcentaje:</h5>
            <input type="number" class="form-control" name="modificar_porcentaje" id="modificar_porcentaje" v-model="popupItem.porcentaje" required min="1" max="100">
          </div>
        </div>
...
export default {
  data() {
    return {
      lista_plazos: [], // Array para almacenar los datos de la API
     ...
     // popupItemNuevo: null,
      ...
      /////para filtrar
      lista_plazos_original: [], // Lista original de usuarios
  ...
  methods: {
    obtenerPlazos() {
      axios.get('http://127.0.0.1:8000/api/plazos/0')
          .then(response => {
           ...
           // this.lista_plazos = response.data["plazos"];

            this.lista_plazos_original = response.data["plazos"];
            this.lista_plazos = [...this.lista_plazos_original]; // Copia la lista original a la lista filtrada
          })
          .catch(error => {
            console.error(error);
          });
    ...
    eliminar_plazos(id){
      ...
      const url = `http://127.0.0.1:8000/api/plazos/delete/${id}`;
      axios.delete(url)
          .then(response => {
            this.obtenerPlazos();
    ...
    guardarCambios() {
      ...
      if (this.popupItem.nombre_plazo === '') {
      ...
      else if (this.popupItem.total_asignado === 0) {
        // Mostrar mensaje de error si el campo está vacío
        this.showErrorMessage = true;
        setTimeout(() => {
          this.showErrorMessage = false; // Ocultar mensaje de error después de unos segundos
      ...
      const url = `http://127.0.0.1:8000/api/plazos/put/${this.popupItem.id}`;
      const payload = {
        nombre_plazo: this.popupItem.nombre_plazo,
        remuneracion: this.popupItem.remuneracion,
        cumplimiento_de_normas: this.popupItem.cumplimiento_de_normas,
        total_asignado: this.popupItem.total_asignado,
        porcentaje: this.popupItem.porcentaje
        // Otros campos a modificar si es necesario
      };

      axios.put(url, payload)
          .then(response => {
            this.obtenerPlazos(); // Actualizar la tabla si es necesario
    ...
    guardar_nuevo_plazo() {
      ...
      if (
          ...
          this.popupItemNuevo.nombre_plazo.trim() === '' ||
      ...
      const url = 'http://127.0.0.1:8000/api/plazos/post/';
      const payload = {
        nombre_plazo: this.popupItemNuevo.nombre_plazo,
        remuneracion: this.popupItemNuevo.remuneracion,
        //cumplimiento_de_normas: this.popupItemNuevo.cumplimiento_de_normas,
        cumplimiento_de_normas: this.popupItemNuevo.cumplimiento_de_normas,
        total_asignado: this.popupItemNuevo.total_asignado,
      ...
      axios.post(url, payload)
          .then(response => {
            ...
            this.popupItemNuevo.nombre_plazo = ''; // Limpiar el campo de texto
            this.popupItemNuevo.remuneracion = 0; // Limpiar el campo de texto
            this.popupItemNuevo.cumplimiento_de_normas = false; // Limpiar el campo de texto
            this.popupItemNuevo.total_asignado = 0; // Limpiar el campo de texto
    ...
    validarNombre(item) {
      ...
      if (!regex.test(item.nombre_plazo)) {
        // Si el valor no coincide con la expresión regular, eliminar caracteres no válidos
        item.nombre_plazo = item.nombre_plazo.replace(/[^a-zA-ZÁáÉéÍíÓóÚúÜü\s]/g, '');
    ...
    aplicarFiltro() {
      if (this.quitarFiltros) {
        ...
        this.lista_plazos = [...this.lista_plazos_original];
      } else {
        // Aplica el filtro por nombre
        const filteredList = this.lista_plazos_original.filter(plazo => {
          const nombreMatch = (!this.filtroNombre || plazo.nombre_plazo.includes(this.filtroNombre));
          return nombreMatch;
        });

        this.lista_plazos = filteredList;
      }
    },
