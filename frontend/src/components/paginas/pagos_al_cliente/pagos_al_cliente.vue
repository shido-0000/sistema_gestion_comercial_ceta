<template >
  ...
  <!--<div class="containerPrueba">-->
    <h1>Lista de pagos al cliente</h1>
    <br>
    ...
    <table class="table" style="font-family: 'Bahnschrift'">
      ...
      <!--  <th scope="col" style="font-weight: bold" align="center">Nombre</th>-->
        ...
        <th scope="col" style="font-weight: bold"  >
          Fecha
          <span class="arrow" @click="ordenarPorValor('fields.fecha')">
              &#8593;&#8595;
            </span>
        </th>
        ...
        <th scope="col" style="font-weight: bold"  >
          Valor
          <span class="arrow" @click="ordenarPorValor('fields.valor')">
              &#8593;&#8595;
            </span>
        </th>
        ...
        <th scope="col" style="font-weight: bold" >
          Detalles del pago
          <span class="arrow" @click="ordenarPorValor('fields.detalles')">
              &#8593;&#8595;
            </span>
        </th>
      ...
      <tr v-for="(item, index) in lista_pagos_al_cliente" :key="index">
        <th scope="row">{{ index + 1 }}</th>
        <td  >{{ item.fields.fecha }}</td>
        <td  >{{ item.fields.valor }}</td>
        <td >{{ item.fields.detalles }}</td>
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup_de_Insertar" style="font-family: 'Bahnschrift'">
      <div class="popup-contenidoPlazos">
        ...
        <div class="form-group col" style="align-items: baseline;">
          ...
          <div class="col-md-12">
            <h5>Detalles:</h5>
            <input type="text" class="form-control" name="insertar_detalles" id="insertar_detalles" v-model="popupItemNuevo.detalles" required >
          </div>
        </div>
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup" style="font-family: 'Bahnschrift'">
      <div class="popup-contenidoPlazos" style="top: 33%">
        ...
        <div class="form-group col" style="align-items: baseline;">
          <div class="col-md-12">
            <h5>Fecha:</h5>
            <input type="date" class="form-control" name="insertar_fecha" id="insertar_fecha" v-model="popupItem.fields.fecha" required >
          </div>
          ...
          <div class="col-md-12">
            <h5>Valor:</h5>
            <input type="number" class="form-control" name="insertar_valor" id="insertar_valor" v-model="popupItem.fields.valor" required min="1">
          </div>
          ...
          <div class="col-md-12">
            <h5>Detalles:</h5>
            <input type="text" class="form-control" name="insertar_detalles" id="insertar_detalles" v-model="popupItem.fields.detalles" required >
          </div>
        </div>
...
export default {
  data() {
    return {
      lista_pagos_al_cliente: [], // Array para almacenar los datos de la API
     ...
     // popupItemNuevo: null,
      ...
      /////para filtrar
      lista_pagos_al_cliente_original: [], // Lista original
  ...
  methods: {
    obtenerPagos_al_cliente() {
      axios.get('http://127.0.0.1:8000/api/pagos_al_cliente/0')
          .then(response => {

            this.lista_pagos_al_cliente = JSON.parse(response.data["pago"]);

            this.lista_pagos_al_cliente_original = JSON.parse(response.data["pago"]);
         //   this.lista_pagos_al_cliente = [...this.lista_pagos_al_cliente_original]; // Copia la lista original a la lista filtrada
          })
          .catch(error => {
            console.error(error);
          });
    ...
    eliminar(id){
      ...
      const url = `http://127.0.0.1:8000/api/pagos_al_cliente/delete/${id}`;
      axios.delete(url)
          .then(response => {
            this.obtenerPagos_al_cliente();
    ...
    guardarCambios() {
      ...
      if (this.popupItem.fecha === null ||
          this.popupItem.valor === 0 ||
          this.popupItem.detalles === ''
      ) {
      ...
      const url = `http://127.0.0.1:8000/api/pagos_al_cliente/put/${this.popupItem.pk}`;
      const payload = {
        fecha: this.popupItem.fields.fecha,
        valor: this.popupItem.fields.valor,
        detalles: this.popupItem.fields.detalles,
      ...
      axios.put(url, payload)
          .then(response => {
            this.obtenerPagos_al_cliente(); // Actualizar la tabla si es necesario
    ...
    guardar_nuevo() {
      ...
      if (this.popupItemNuevo.fecha === null ||
          this.popupItemNuevo.valor === 0 ||
          this.popupItemNuevo.detalles === ''
      ) {
      ...
      const url = 'http://127.0.0.1:8000/api/pagos_al_cliente/post/';
      const payload = {
        fecha: this.popupItemNuevo.fecha,
        valor: this.popupItemNuevo.valor,
        detalles: this.popupItemNuevo.detalles,
      };
    ...
    ordenarPorValor(columna) {
      ...
      // Lógica de ordenamiento aquí
      this.lista_pagos_al_cliente.sort((a, b) => {
        const factor = this.ordenAsc ? 1 : -1;
    ...
    aplicarFiltro() {
      ...
      //  this.lista_pagos_al_cliente = [...this.lista_valor_del_contrato_por_los_servicios_comercializados_original];
      } else {
        // Aplica el filtro por nombre
        const filteredList = this.lista_pagos_al_cliente_original.filter(valor_del_contrato => {
