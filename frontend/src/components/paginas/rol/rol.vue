...
<br><br><br>
  ...
  <!--<div class="containerPrueba">-->
    ...
    <div class="alert alert-danger" role="alert" v-if="menssage_no_se_puede_eliminar_rol">
      Error, no puede eliminar a este rol
    </div>
    ...
    <table class="table" style="font-family: 'Bahnschrift'">
      ...
      <!--  <th scope="col" style="font-weight: bold" align="center">Nombre</th>-->
        ...
        <th scope="col" style="font-weight: bold" align="center">
          Nombre
      ...
      <tr v-for="(item, index) in lista_rol" :key="index">
        <th scope="row">{{ index + 1 }}</th>
        <td align="center">{{ item.nombre_rol }}</td>
        ...
        <td>
          <div class="fila">
            ...
            <button class="btn btn-icon-text elemento2" @click="eliminar_rol(item.id)" style="margin-left: 20px;border-color: red; background-color: red" type="submit">
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup_de_Insertar" style="font-family: 'Bahnschrift'">
      <div class="popup-contenidoRol" style="top: 33%">
        ...
        <input type="text" class="form-control" name="insertar_nombre" id="insertar_nombre" v-model="popupItemNuevo.nombre_rol" required @input="validarNombre(popupItemNuevo)">

        <br>
        <div class="fila">
          <button class="btn btn-icon-text elemento3" @click="guardar_nuevo_rol" type="submit">
...
export default {
  data() {
    return {
      lista_rol: [], // Array para almacenar los datos de la API
  ...
  methods: {
    ...
    eliminar_rol(id){
      if (this.nombre_del_rol_para_navbar === 'Especialista de sistemas') {
        ...
        this.menssage_no_se_puede_eliminar_rol = true;
        // Mostrar un mensaje de error
        setTimeout(() => {
          this.menssage_no_se_puede_eliminar_rol = false; // Ocultar mensaje de error después de unos segundos
        }, 3000);

        return; // Salir de la función sin eliminar el rol
      }

      const url = `http://127.0.0.1:8000/api/roles/delete/${id}`;
      axios.delete(url)
          .then(response => {
            this.obtener_rol();
    ...
    guardarCambios() {
      ...
      const url = `http://127.0.0.1:8000/api/roles/put/${this.popupItem.id}`;
      const payload = {
        nombre_rol: this.popupItem.nombre_rol,
        // Otros campos a modificar si es necesario
      };

      axios.put(url, payload)
          .then(response => {
            this.obtener_rol(); // Actualizar la tabla si es necesario
    ...
    guardar_nuevo_rol() {
      ...
      if (
          ...
          this.popupItemNuevo.nombre_rol.trim() === ''
      ) {
      ...
      const url = 'http://127.0.0.1:8000/api/roles/post/';
      const payload = {
        nombre_rol: this.popupItemNuevo.nombre_rol,
      };
