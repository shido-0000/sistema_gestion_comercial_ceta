<template>
  ...
  <div class="container-md-10" style="margin: 20px;font-family: 'Bahnschrift'">
    ...
    <div class="form-group row" style="align-items: baseline;font-family: 'Bahnschrift'">
      ...
      <table class="table" style="font-family: 'Bahnschrift'">
        ...
        <tr>
          ...
          <th scope="col" style="font-weight: bold"  >
            Nombre
            <span class="arrow" @click="ordenarPor('nombre')">
              &#8593;&#8595;
            </span>
          </th>
            <!-- <th scope="col" style="font-weight: bold" align="center">Cantidad</th>-->
          <th scope="col" style="font-weight: bold"  >
            Cantidad
            <span class="arrow" @click="ordenarPorValor('cantidad')">
             ...
             <th scope="col" style="font-weight: bold"  >
               Tipo
               <span class="arrow" @click="ordenarPor('tipo')">
                 &#8593;&#8595;
               </span>
             </th>
      ...
      <tr v-for="(item, index) in lista_normas" :key="index">
        ...
        <td>
          <div class="fila">
            ...
            <button class="btn btn-icon-text elemento2" @click="eliminar_normas(item.id)" style="margin-left: 20px;border-color: red; background-color: red" type="submit">
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup_de_Insertar">
    <!--<div class="popup" v-if="mostrarPopup_de_Insertar">-->
      <div class="popup-contenidoNorma">
        ...
        <h2>Crear nueva norma</h2>

        <br>
        <h5>Nombre del la norma:</h5>

        <input type="text" class="form-control" name="insertar_nombre" id="insertar_nombre" v-model="popupItemNuevo.nombre" required @input="validarNombre(popupItemNuevo) ">
        <br>
        <div>
          <h5>Cantidad:</h5>
          <input type="number" class="form-control" name="insertar_cantidad" id="insertar_cantidad" v-model="popupItemNuevo.cantidad" required>
        </div>
        <br>
        <div>
          <h5>Tipo:</h5>
          <input type="text" class="form-control" name="insertar_tipo" id="insertar_tipo" v-model="popupItemNuevo.tipo" required>
        </div>
        <br>
        <div class="fila">
          <button class="btn btn-icon-text elemento3" @click="guardar_nueva_norma" type="submit">
    ...
    <div class="popup modal fade show d-block" v-if="mostrarPopup" style="font-family: 'Bahnschrift'">
      ...
      <div class="popup-contenidoNorma">
        ...
        <input type="text" class="form-control" name="modificar_nombre" id="modificar_nombre" v-model="popupItem.nombre" required @input="validarNombre(popupItem) ">
        <br>
        <div>
          <h5>Cantidad:</h5>
          <input type="number" class="form-control" name="modificar_cantidad" id="modificar_cantidad" v-model="popupItem.cantidad" required>
        </div>

        <br>
        <div>
          <h5>Tipo:</h5>
          <input type="text" class="form-control" name="modificar_tipo" id="modificar_tipo" v-model="popupItem.tipo" required>
        </div>
...
export default {
  data() {
    return {
      lista_normas: [], // Array para almacenar los datos de la API
      ...
      /////para filtrar
      lista_normas_original: [], // Lista original de usuarios
  ...
  methods: {
    obtenerNormas() {
      axios.get('http://127.0.0.1:8000/api/normas/0')
          .then(response => {
            // this.tipos_productos = response.data["Tipos de productos"]; // Actualizar los datos en la tabla
            //console.log(response);
          //  this.lista_normas = response.data["norma"];


            this.lista_normas_original = response.data["norma"];
            this.lista_normas = [...this.lista_normas_original]; // Copia la lista original a la lista filtrada
    ...
    eliminar_normas(id){
      ...
      const url = `http://127.0.0.1:8000/api/normas/delete/${id}`;
      axios.delete(url)
          .then(response => {
            this.obtenerNormas();
    ...
    guardarCambios() {
      ...
      else if (this.popupItem.cantidad === 0) {
        // Mostrar mensaje de error si el campo está vacío
        this.showErrorMessage = true;
        setTimeout(() => {
          this.showErrorMessage = false; // Ocultar mensaje de error después de unos segundos
      ...
      const url = `http://127.0.0.1:8000/api/normas/put/${this.popupItem.id}`;
      const payload = {
        nombre: this.popupItem.nombre,
        cantidad: this.popupItem.cantidad,
        tipo: this.popupItem.tipo
        // Otros campos a modificar si es necesario
      };

      axios.put(url, payload)
          .then(response => {
            this.obtenerNormas(); // Actualizar la tabla si es necesario
    ...
    guardar_nueva_norma() {
      ...
      if (
          !this.popupItemNuevo.nombre ||
          this.popupItemNuevo.nombre.trim() === '' ||
          this.popupItemNuevo.cantidad === 0 ||
          this.popupItemNuevo.tipo.trim() === '') {
      ...
      const url = 'http://127.0.0.1:8000/api/normas/post/';
      const payload = {
        nombre: this.popupItemNuevo.nombre,
        cantidad: this.popupItemNuevo.cantidad,
        tipo: this.popupItemNuevo.tipo
      };
    ...
    aplicarFiltro() {
      if (this.quitarFiltros) {
        ...
        this.lista_normas = [...this.lista_normas_original];
      } else {
        // Aplica el filtro por nombre
        const filteredList = this.lista_normas_original.filter(norma => {
          const nombreMatch = (!this.filtroNombre || norma.nombre.includes(this.filtroNombre));
          return nombreMatch;
        });

        this.lista_normas = filteredList;
      }
    },
